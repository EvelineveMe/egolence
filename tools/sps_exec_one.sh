...
log_receipt() {
  ...
  # Trigger a system event for OpenClaw to pick up
  openclaw system event --text "SPS $status: $msg" --mode now || true
}

# --- SPS_WRITE / SPS_APPEND ---
...
" && {
    ...
    log_receipt "FAILED" "$ACTION — string not found"
  }
...
if echo "$ACTION" | grep -E "(;|&&|\|\||\||>|<|<<|>>|\\|\$\(|\`)" >/dev/null; then
  log_receipt "BLOCKED" "$ACTION — invalid tokens"
  tail -n +2 "$QUEUE_FILE" > "$QUEUE_FILE.tmp" && mv "$QUEUE_FILE.tmp" "$QUEUE_FILE"
  exit 0
fi

log_receipt "BLOCKED" "$ACTION — non-SPS action disabled"
tail -n +2 "$QUEUE_FILE" > "$QUEUE_FILE.tmp" && mv "$QUEUE_FILE.tmp" "$QUEUE_FILE"
exit 0
